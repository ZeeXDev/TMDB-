...
BOT_TOKEN = "7703477452:AAFXr-oSL1yR2CouVqxaMd3lxH_4z66In4A"

# Initialisation du traducteur
translator = GoogleTranslator(source='auto', target='fr')

# Dictionnaires de genres et emojis
GENRES_EMOJI = {
...
async def traduire_texte(texte, src='auto', dest='fr'):
    ...
    try:
        ...
        if src == 'auto':
            traducteur = GoogleTranslator(source='auto', target=dest)
        else:
            traducteur = GoogleTranslator(source=src, target=dest)
        
        texte_traduit = traducteur.translate(texte_propre)
        return texte_traduit
        
    except Exception as e:
        logger.error(f"Erreur de traduction Deep Translator: {e}")
...
def create_selection_buttons(results, source="tmdb"):
    ...
    for result in results:
        if source == "tmdb":
            ...
            # PRIORITÉ AU TITRE ANGLAIS maintenant
            title = title_obj.get("english") or title_obj.get("userPreferred") or title_obj.get("romaji") or "Sans titre"
...
async def formater_reponse(details, media_type, season_num=None, year=None):
    ...
    # Traduire le résumé si nécessaire avec Deep Translator
...
async def formater_reponse_anilist(details):
    ...
    # PRIORITÉ AU TITRE ANGLAIS maintenant
    titre = title_obj.get("english") or title_obj.get("userPreferred") or title_obj.get("romaji") or "Inconnu"
    titre_original = title_obj.get("native") or title_obj.get("romaji") or titre
    titre_romaji = title_obj.get("romaji") or "Inconnu"
    ...
    # Traduire le résumé depuis l'anglais si nécessaire avec Deep Translator
    ...
    texte = (
        f"<blockquote>{is_adult}🇯🇵 𝗔𝗻𝗶𝗺𝗲: <a href='t.me/WorldZPrime'>{escape(titre)}</a> / <a href='t.me/WorldZPrime'>{escape(titre)}</a> / <a href='t.me/WorldZPrime'>{escape(titre_original)}</a> / <a href='t.me/WorldZPrime'>{escape(titre_romaji)}</a></blockquote>\n\n"
    ...
    # Récupérer le poster depuis Kitsu
...
async def send_anime_info_tmdb_poster(message, details):
    """Envoie les informations d'anime AniList avec image HD depuis TMDB"""
    formatted = await formater_reponse_anilist(details)
    
    # Pour /ani2, on cherche d'abord sur TMDB pour le poster
    title_obj = details.get("title", {})
    titre = title_obj.get("english") or title_obj.get("userPreferred") or title_obj.get("romaji") or "Inconnu"
    
    # Rechercher le média sur TMDB
    tmdb_results = await rechercher_media_multiple(titre)
    tmdb_poster_url = None
    
    if tmdb_results:
        # Prendre le premier résultat TMDB
        first_result = tmdb_results[0]
        tmdb_details = await get_media_details(first_result["media_type"], first_result["id"])
        if tmdb_details and tmdb_details.get('poster_path'):
            tmdb_poster_url = f"https://image.tmdb.org/t/p/original{tmdb_details['poster_path']}"
    
    # Utiliser le poster TMDB si trouvé, sinon utiliser Kitsu comme fallback
    poster_url = tmdb_poster_url or formatted["poster"]
    ...
    if poster_url:
        ...
        try:
            ...
            logger.error(f"Erreur envoi image AniList/TMDB: {e}")
            # Dernier fallback: envoyer sans image
            await message.reply_text(
                limiter_texte(formatted["texte"]),
                parse_mode=enums.ParseMode.HTML
        ...
        # Si pas de poster du tout, essayer avec l'image AniList en fallback
        poster_url = details.get("coverImage", {}).get("extraLarge")
        if poster_url:
            image_data = await download_image_hd(poster_url)
...
async def demarrage(client, message):
    try:
        await message.reply_text(
            ...
            "- <code>/ani &lt;titre&gt;</code> pour rechercher un anime sur AniList (posters Kitsu)\n"
            "- <code>/ani2 &lt;titre&gt;</code> pour rechercher un anime sur AniList (posters TMDB)\n\n"
            "<b>Exemples :</b>\n"
            "- Le Comte de Monte-Cristo\n"
            "- Avatar\n"
            "- <code>/ani Naruto</code>\n"
            "- <code>/ani2 Attack on Titan</code>\n\n"
...
@app.on_message(filters.command("ani2"))
async def search_anilist_tmdb_poster(client, message):
    try:
        if len(message.command) < 2:
            return await message.reply_text("❌ Utilisation: <code>/ani2 &lt;titre de l'anime&gt;</code>", parse_mode=enums.ParseMode.HTML)
        ...
        msg = await message.reply_text("🔍 Recherche en cours sur AniList (posters TMDB)...")
        ...
        if len(results) == 1:
            ...
            details = await get_anime_details_anilist(result["id"])
            if details:
                await send_anime_info_tmdb_poster(message, details)
                await msg.delete()
            ...
            await msg.edit_text(
                f"🎭 <b>J'ai trouvé {len(results)} animes sur AniList pour \"{requete}\" (posters TMDB)</b>\n\n"
                "📋 <i>Sélectionnez celui qui vous intéresse :</i>",
                reply_markup=create_selection_buttons(results, "anilist"),
                parse_mode=enums.ParseMode.HTML
            )
        ...
        logger.error(f"Erreur dans la commande ani2: {e}")
        await message.reply_text("❌ Une erreur s'est produite lors de la recherche.")

@app.on_message(filters.text & ~filters.command(["start", "help", "aide", "ani", "ani2"]))
...
async def select_callback(client, callback):
    try:
        ...
        if source == "anilist":
            ...
            # Vérifier si c'est une recherche normale ou ani2
            chat_data = recherches_en_cours.get(callback.message.chat.id, {})
            if chat_data.get("source") == "anilist_tmdb":
                await send_anime_info_tmdb_poster(callback.message, details)
            else:
